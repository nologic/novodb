...
make_helper() {
  ...
  plutil -insert "CFBundleDisplayName" -string "${helper_feature}" "${feature_info_plist}"
  plutil -replace "CFBundleExecutable" -string "${helper_feature}" "${feature_info_plist}"

  local plist_out_tmp=$( mktemp /tmp/plist.XXXX )

  cfbundleid="`plutil -extract "CFBundleIdentifier" binary1 "${feature_info_plist}" -o "${plist_out_tmp}" && cat "${plist_out_tmp}"`"
  feature_cfbundleid="${cfbundleid}.${feature}"
  plutil -replace "CFBundleIdentifier" -string "${feature_cfbundleid}" "${feature_info_plist}"

  cfbundlename="$(plutil -extract "CFBundleName" binary1 "${feature_info_plist}" -o "${plist_out_tmp}" && cat "${plist_out_tmp}")"
  feature_cfbundlename="${cfbundlename} ${feature}"
  plutil -replace "CFBundleName" -string "${feature_cfbundlename}" "${feature_info_plist}"
